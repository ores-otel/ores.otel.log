...
path = root / "sdk/ruby/lib/oresoftware/next_loggers.rb"
text = path.read_text()

old_storage = '''    def current_context
...
'''
new_storage = '''    def current_context
...
if text.count(old_storage) != 1:
    raise SystemExit("Ruby context storage fragment drifted")
text = text.replace(old_storage, new_storage, 1)

marker = '''    def normalize_context(context)
'''
deep_snapshot = '''    MAX_CONTEXT_SNAPSHOT_DEPTH = 128
...
if text.count(marker) != 1:
    raise SystemExit("Ruby normalize_context marker drifted")
text = text.replace(marker, deep_snapshot + marker, 1)

old_init = '''        self.fields = (fields || {}).each_with_object({}) do |(key, value), out|
          out[key.to_s] = value
        end.freeze
        self.tags = Array(tags).map(&:to_s).uniq.freeze
'''
new_init = '''        normalized_fields = (fields || {}).each_with_object({}) do |(key, value), out|
          out[key.to_s] = value
        end
...
if text.count(old_init) != 1:
    raise SystemExit("Ruby Context initializer fragment drifted")
text = text.replace(old_init, new_init, 1)
path.write_text(text)

test_path = root / "sdk/ruby/test/context_fiber_isolation_test.rb"
test_path.write_text(r'''# frozen_string_literal: true
