...
use std::{collections::BTreeMap, rc::Rc, sync::{Arc, atomic::{AtomicU64, Ordering}}};
...
fn random_hex(bytes: usize) -> Result<String, String> {
    ...
    window.crypto().map_err(|_| "browser crypto is unavailable")?
        .get_random_values_with_u8_array(&mut data).map_err(|_| "browser entropy failed")?;
...
pub fn child_trace(parent: &TraceParent) -> Result<TraceParent, String> {
    parent.child(&random_hex(8)?).map_err(|error| error.to_string())
}
...
impl BrowserLogger {
    ...
    where F: Fn(&LogRecord) -> Result<(), String> + 'static {
        if service.len() > 128 { return Err("service name exceeds 128 bytes".into()); }
        let prefix = random_hex(16)?;
        let counter = Arc::new(AtomicU64::new(1));
        let logger = Logger::new(service)?
            .with_clock(|| String::from(js_sys::Date::new_0().to_iso_string()))
            .with_id_factory(move || format!("{prefix}-{}", counter.fetch_add(1, Ordering::Relaxed)));
        Ok(Self { logger, sink: Rc::new(sink) })
    }

    pub fn log(&self, level: LogLevel, message: &str, trace: Option<&TraceParent>) -> Result<LogRecord, String> {
        let context = trace.map(|trace| LogContext {
            trace_id: Some(trace.trace_id().into()), span_id: Some(trace.span_id().into()),
            trace_flags: trace.flags(), ..Default::default()
        });
        let record = self.logger.log(level, message, context.as_ref(), BTreeMap::new())?;
        (self.sink)(&record)?;
        Ok(record)
    }
...
pub fn traced_request(url: &str, init: &RequestInit, trace: &TraceParent) -> Result<Request, JsValue> {
    let window = web_sys::window().ok_or_else(|| JsValue::from_str("browser Window is unavailable"))?;
    let request = Request::new_with_str_and_init(url, init)?;
    let target = Url::new(&request.url())?;
    if !matches!(target.protocol().as_str(), "https:" | "http:")
        || target.origin() != window.location().origin()? || !target.username().is_empty() || !target.password().is_empty() {
        return Err(JsValue::from_str("trace propagation requires the page's exact HTTP(S) origin"));
    }
