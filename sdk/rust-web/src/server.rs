...
use axum::{extract::{Request, State}, http::HeaderValue, middleware::{from_fn_with_state, Next}, response::Response, Router};
use next_loggers::{json, with_log_context_async, LogContext, LogRecord, Logger, LoggerError, Options, Transport};
...
impl Transport for JsonStdout {
    fn write(&self, record: &LogRecord) -> Result<(), LoggerError> {
        ...
        line.push('\n');
        std::io::stdout().lock().write_all(line.as_bytes()).map_err(|_| LoggerError("telemetry stdout write failed".into()))
    }
}
...
where S: Clone + Send + Sync + 'static {
    let logger = Logger::new(Options { app_name: service.into(), console: false, ..Options::default() }
...
fn context_for(headers: &axum::http::HeaderMap) -> (TraceParent, Option<TraceParent>) {
    ...
    let parent = if values.next().is_none() {
        first.and_then(|value| value.to_str().ok()).and_then(|value| value.parse::<TraceParent>().ok())
    } else { None };
...
async fn correlate(State(logger): State<Logger>, mut request: Request, next: Next) -> Response {
    ...
    let method = match request.method().as_str() {
        "GET" | "HEAD" | "POST" | "PUT" | "DELETE" | "CONNECT" | "OPTIONS" | "TRACE" | "PATCH" => request.method().as_str().to_owned(),
        _ => "OTHER".into(),
    };
    let mut context = LogContext {
        trace_id: Some(trace.trace_id().into()), span_id: Some(trace.span_id().into()),
        trace_flags: trace.flags(), remote: Some(false), ..Default::default()
    };
    if let Some(parent) = parent {
        context.fields.insert("otel.parent_span_id".into(), json!(parent.span_id()));
    }
    ...
    with_log_context_async(context, async move {
        ...
        if let Ok(header) = HeaderValue::from_str(&trace.to_string()) {
            response.headers_mut().entry("traceparent").or_insert(header);
        }
...
mod tests {
    use super::*;
    use axum::{body::Body, http::{HeaderMap, Request as HttpRequest}, routing::get};
    ...
    fn missing_or_duplicate_header_starts_unsampled_root() {
        ...
        assert!(parent.is_none()); assert!(!root.sampled());
    ...
    fn valid_parent_gets_new_span() {
        ...
        assert_ne!(child.span_id(), parent.span_id()); assert!(!child.sampled());
    }
    #[tokio::test]
    async fn scopes_handler_and_preserves_response() {
        let app = install(Router::new().route("/", get(|| async {
            ...
            assert!(context.logged_in_user.is_empty()); assert!(context.baggage.is_empty());
            (axum::http::StatusCode::ACCEPTED, "ok")
        })), "test-server");
        let response = app.oneshot(HttpRequest::builder().uri("/").header("traceparent", PARENT)
            .header("baggage", "authorization=secret").body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(response.status(), axum::http::StatusCode::ACCEPTED);
        let child: TraceParent = response.headers()["traceparent"].to_str().unwrap().parse().unwrap();
