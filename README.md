...
## Kubernetes and Cloud Run OTLP sidecars

The `oresoftware/otel-k8s-sidecar` and `oresoftware/otel-cloud-run-sidecar`
Zed targets package hardened, opt-in
OpenTelemetry Collector sidecars for application-owned OTLP logs, metrics, and
traces. They listen on workload-local loopback, apply bounded memory, queue, retry,
...
The Kubernetes target includes a machine-readable, exact-head adoption catalog for 17
