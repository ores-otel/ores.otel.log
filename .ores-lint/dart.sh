...
OUT=$(mktemp) || exit 0
RC=0
# `dart analyze` exits non-zero when it finds issues, so `machine || human`
# would concatenate both formats and double-count. Prefer machine output;
# fall back to human only when the machine flag is unsupported.
( cd "$ROOT" && $ANALYZE --format=machine ) >"$OUT" 2>&1 || RC=$?
if ! grep -qE '^(ERROR|WARNING|INFO)\|' "$OUT" && ! grep -q 'No issues found!' "$OUT"; then
  if [ ! -s "$OUT" ] || grep -qiE 'unrecognized|unknown option|invalid option|format' "$OUT"; then
    RC=0
    ( cd "$ROOT" && $ANALYZE ) >"$OUT" 2>&1 || RC=$?
  fi
fi
