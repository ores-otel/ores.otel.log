...
export const requireSendRule = {
  ...
  create(context) {
    ...
    const isOresSuppressed = (node) => {
      if (!sourceCode || !node?.loc) return false;
      const line = node.loc.start.line;
      for (const comment of sourceCode.getAllComments()) {
        const text = String(comment.value || '');
        if (!/ores-lint-disable/.test(text)) continue;
        if (/ores-lint-disable-file\s+require-send/.test(text)) return true;
        const commentLine = comment.loc?.end?.line;
        if (/ores-lint-disable-next-line\s+require-send/.test(text) && commentLine === line - 1) return true;
        if (/ores-lint-disable-line\s+require-send/.test(text) && commentLine === line) return true;
      }
      return false;
    };

    const reportMissing = (node) => {
      if (isOresSuppressed(node)) return;
...
const plugin = { meta: { name: 'ores-lint', version: '1.3.1' }, rules };
