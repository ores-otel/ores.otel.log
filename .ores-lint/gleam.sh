...
TOMLS=$(cd "$ROOT" && find . -maxdepth "$ORES_LINT_DEPTH" \
    \( -name node_modules -o -name target -o -name .git -o -name build -o -name vendor -o -name .vendor -o -name .ores-lint -o -name .r2g -o -name deps \) -prune -o \
...
for c in $TOMLS; do
  ...
  ( cd "$cdir" && gleam format --check $FMT_ARGS ) >"$OUT" 2>&1 || FMT_RC=$?
  ( cd "$cdir" && gleam check ) >>"$OUT" 2>&1 || CHK_RC=$?
  [ "$FMT_RC" -eq 0 ] && [ "$CHK_RC" -eq 0 ] || RC=1
  RAN=$((RAN + 1))
  if [ "$RC" -ne 0 ] && ! grep -qiE 'error|warning|not been formatted|not formatted|Which files to format' "$OUT"; then
    FAILED="$FAILED
  $c (exit $RC): $(sed -n '1,2p' "$OUT" | tr '\n' ' ' | cut -c1-140)"
    rm -f "$OUT"
    continue
  fi
  # Gleam colour-codes "error:" (CSI between "error" and ":"), so strip before aggregating.
  STRIPPED=$(mktemp)
  sed $'s/\033\\[[0-9;]*[A-Za-z]//g' "$OUT" > "$STRIPPED"
  if [ "$c" = "." ]; then cat "$STRIPPED" >> "$RAW"; else sed "s|^|$c/|" "$STRIPPED" >> "$RAW"; fi
  rm -f "$OUT" "$STRIPPED"
done
...
/error:|warning:|not been formatted|[Nn]ot formatted/ {
  msg = $0
  n++
