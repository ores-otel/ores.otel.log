...
function walkSourceFiles(dir, prefix, depth, acc) {
  if (depth < 0) return;
  let entries;
  try {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) walkSourceFiles(join(dir, e.name), rel, depth - 1, acc);
    else if (/\.(?:rs|dart|gleam)$/.test(e.name)) acc.push(rel);
  }
}

function listedFiles() {
  const raw = process.env.ORES_LINT_REQUIRE_SEND_FILES;
  if (!raw) return null;
  return raw.split(/[\n,]/).map((s) => s.trim()).filter(Boolean);
}

function gitLsFilesAtRoot() {
  try {
    const top = execFileSync('git', ['-C', ROOT, 'rev-parse', '--show-toplevel'], {
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore'],
    }).trim();
    if (resolve(top) !== resolve(ROOT)) return null;
...
function trackedFiles() {
  ...
  const explicit = listedFiles();
  if (explicit) return explicit.filter((rel) => keepRel(rel, nested));
  const output = gitLsFilesAtRoot();
  const candidates = output
    ? output.split('\0').filter(Boolean)
    : (() => {
      const acc = [];
      const depth = Math.max(4, Number(process.env.ORES_LINT_DEPTH || 8) + 4);
      walkSourceFiles(ROOT, '', depth, acc);
      return acc;
    })();
  return candidates.filter((rel) => keepRel(rel, nested));
}
